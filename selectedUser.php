...
$sql = "SELECT * FROM tsf.user where id=$from "; 
$result = $mysqli->query($sql);
$rows=$result->fetch_assoc();


$sql = "SELECT * from tsf.user where id=$to";
$result = $mysqli->query($sql);
...
if (($amount)<0)
{
 ...
 else if($amount >$rows['balance']) 
 {
  ...
  $sql = "UPDATE tsf.user set balance=$newbalance where id=$from";
  mysqli_query($mysqli,$sql);


  // adding amount to reciever's account
  $newbalance = $rows1['balance'] + $amount;
  $sql = "UPDATE tsf.user set balance=$newbalance where id=$to";
  mysqli_query($mysqli,$sql);
  
  $sender = $rows['name'];
  $receiver = $rows1['name'];
  $sql = "INSERT INTO tsf.transaction(`sender`, `receiver`, `amount`) VALUES ('$sender','$receiver','$amount')";
...
<body>
  ...
  <div class="container">
    ...
    <?php
                ...
                $sid=$_GET['id'];
                $sql = "SELECT * FROM  tsf.user where id=$sid";
                $result = $mysqli->query($sql); 
